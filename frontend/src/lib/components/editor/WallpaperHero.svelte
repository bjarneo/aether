<script lang="ts">
    ...
    let blurred = $derived(!!blurredPath);
    ...
    // Toggle a heavy Gaussian-blurred variant as the applied wallpaper. The
    // blurred copy is generated by the backend into its cache; the source
    // file is never modified, so Extract keeps sampling the original.
    async function handleToggleBlur() {
        const source = getWallpaperPath();
        if (!source || isBlurring) return;
        if (getBlurredWallpaperPath()) {
            clearBlurredWallpaper();
            showToast('Blur removed — the original will be applied');
            return;
        }
        isBlurring = true;
        try {
            ...
            setBlurredWallpaper(source, path);
            showToast(
                'Wallpaper blurred — Apply uses the blurred copy; palette still extracts from the original'
            );
        } catch {
            showToast("Couldn't blur wallpaper");
        } finally {
            isBlurring = false;
        }
    }
        ...
        >
            ...
            <div class="pointer-events-auto flex shrink-0 items-center gap-1">
                ...
                <button
                    ...
                    disabled={isBlurring}
                    title={blurred
                        ? 'Remove blur — apply the original image again'
                        : 'Heavy blur — the applied wallpaper becomes a blurred copy; palette still extracts from the original'}
                    aria-label={blurred
                        ? 'Remove blur'
